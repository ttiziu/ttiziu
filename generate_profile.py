...
def header_parts(login: str) -> list[tuple[str, str]]:
    return [("key", f"{login}@github"), ("muted", "  " + "─" * 47)]


def loc_parts(loc: dict) -> list[tuple[str, str]]:
    prefix = dotted_parts("Lines of Code", fmt_int(loc["net"]), STATS_COL)
    return prefix + [
        ("muted", "   ("),
...
def repos_parts(owned: int, contributed: int, stars: int) -> list[tuple[str, str]]:
    return [
        *dotted_parts("Repos", fmt_int(owned), STATS_COL),
        ("muted", " {Contributed: "),
        ("value", fmt_int(contributed)),
        ("muted", "} | "),
        *dotted_parts("Stars", fmt_int(stars), STATS_COL),
    ]


def pair_parts(left_label: str, left_val: str, right_label: str, right_val: str) -> list[tuple[str, str]]:
    return [
        *dotted_parts(left_label, left_val, STATS_COL),
        ("muted", " | "),
        *dotted_parts(right_label, right_val, STATS_COL),
    ]
...
def build_lines(
    ...
    return [
        ...
        repos_parts(user["repositories"]["totalCount"], contributed, stars),
        pair_parts(
            "Commits",
            fmt_int(loc["commits"]),
            "Followers",
