...
def load_star() -> list[str]:
    if not STAR_PATH.exists():
        return []
    lines = STAR_PATH.read_text(encoding="utf-8").splitlines()
    ...
    return [line.rstrip().ljust(width) for line in lines]


def with_logo(rows: list[list[tuple[str, str]]]) -> list[list[tuple[str, str]]]:
    star = load_star()
    if not star:
        return rows
    logo_w = len(star[0])
    top = max((len(rows) - len(star)) // 2, 0)
    padded: list[str | None] = [None] * top + star
    if len(padded) < len(rows):
...
def render_readme(login: str, width: int, height: int) -> str:
...
<picture>
  ...
  <img alt="{login}@github" src="{base}/profile-light.svg" width="{width}" height="{height}">
</picture>
...
def write_outputs(
    ...
    README_PATH.write_text(
        render_readme(username(config), svg_w, svg_h), encoding="utf-8"
    )
